...
- **Empty Bubble Flicker Protection & Timeupdate Guard**: Completely hid empty subtitle containers (`.holdtranslate-yt-sub:empty`, `data-empty="true"`) to prevent empty box flicker during seek transitions; added throttled `timeupdate` synchronization guard to ensure seamless continuous subtitle translation across all player states.
