...
(function () {
  ...
  let ytObserverHeartbeatTimer = null;
  ...
  // Early start for video subtitles without waiting for storage async delay
  ...
  function getActiveTimedTextTranslation(curMs, originalText) {
    ...
    if (ytSentenceCues && ytSentenceCues.length) {
      const candidates = ytSentenceCues.filter(s => curMs >= s.start - 350 && curMs <= s.end + 450);
      for (const sent of candidates) {
        const trans = sent.translation || getCachedSub(sent.text);
        if (sent && trans) {
          if (originalText) {
  ...
  function handleYouTubeCaptions(captionContainer) {
    ...
    let windows = [];
    if (captionContainer && captionContainer.matches && captionContainer.matches('.caption-window')) {
      windows = [captionContainer];
    } else if (captionContainer && typeof captionContainer.querySelectorAll === 'function') {
      windows = Array.from(captionContainer.querySelectorAll('.caption-window'));
    }

    if (!windows.length) {
      const docWindows = document.querySelectorAll('.caption-window');
      if (docWindows.length) {
        windows = Array.from(docWindows);
      }
    }
    ...
    windows.forEach((win) => {
      ...
      // E. Seamless Retention & Anti-Flicker Protection during live translation / ASR rolling words:
      // While native captions are actively showing, DO NOT HIDE subEl!
      // Keep existing translation visible until the new translation arrives, eliminating blank flashes.
      const hasExistingDisplay = Boolean(subEl.dataset.translatedText && subEl.textContent && subEl.dataset.empty !== 'true');
  ...
  function startYouTubeObserver() {
    const attachObserver = () => {
      hookVideoEvents();
      // Anchor to stable player container that is NEVER destroyed on seek
      const target = document.querySelector('#movie_player') ||
                     document.querySelector('.html5-video-player') ||
                     document.querySelector('#player') ||
                     document.body;

      if (target && target !== ytObservedTarget) {
        if (ytCaptionObserver) {
          try { ytCaptionObserver.disconnect(); } catch (e) {}
        }
        ytObservedTarget = target;
        ytCaptionObserver = new MutationObserver((mutations) => {
          if (!config.videoSubtitlesEnabled) return;
          const hasExternalMutation = mutations.some(m => {
            const t = m.target;
            if (t && t.nodeType === 1 && t.classList && t.classList.contains('holdtranslate-yt-sub')) return false;
            if (t && t.parentElement && t.parentElement.classList && t.parentElement.classList.contains('holdtranslate-yt-sub')) return false;
            return true;
          });
    ...
    // Periodic safety heartbeat to re-anchor observer if player DOM reloads
    if (!ytObserverHeartbeatTimer) {
      ytObserverHeartbeatTimer = setInterval(() => {
    ...
    if (!window.__HOLD_TRANSLATE_YT_HOOKED__) {
      ...
      window.addEventListener('yt-navigate-finish', () => {
        ...
        try {
          window.postMessage({ source: 'holdtranslate-content', type: 'REQUEST_TRACKS' }, '*');
        } catch (e) {}
        setTimeout(attachObserver, 500);
      }, { passive: true });
      window.addEventListener('fullscreenchange', () => setTimeout(attachObserver, 300), { passive: true });
    }
  }

  // --- YouTube TimedText Parsing & Prefetching Engine ---

  function getYouTubeTlang(targetLang) {
    if (!targetLang || targetLang === 'auto' || targetLang === 'zh-CN' || targetLang === 'zh') {
      return 'zh-Hans';
    }
    if (targetLang === 'zh-TW') {
      return 'zh-Hant';
  ...
  function handleInterceptedTimedText(url, rawText) {
    ...
    // IMPORTANT: DO NOT overwrite full-track data if ytFullTrackCues already has data!
    if (ytFullTrackCues && ytFullTrackCues.length >= cues.length) {
      // Cross-fill translations from ytFullTrackCues to new cues
      cues.forEach(newCue => {
        const existing = ytFullTrackCues.find(tc => Math.abs(tc.start - newCue.start) < 500);
        if (existing && existing.translation) {
          newCue.translation = existing.translation;
          setCachedSub(newCue.text, existing.translation);
        }
      });
      // Merge any cues into ytFullTrackCues if not present
      cues.forEach(newCue => {
        const exists = ytFullTrackCues.some(tc => Math.abs(tc.start - newCue.start) < 200);
        if (!exists) {
          ytFullTrackCues.push(newCue);
        }
      });
      ytFullTrackCues.sort((a, b) => a.start - b.start);
      ytTimedTextCues = ytFullTrackCues;
    } else {
      ytTimedTextCues = cues;
      if (!ytFullTrackCues || ytFullTrackCues.length < cues.length) {
        ytFullTrackCues = [...cues];
      }
    }

    ytCurrentTrackUrl = url;

    // If intercepted url does not have tlang, try fetching tlang track
    if (!url.includes('tlang=')) {
      const cleanBase = url.replace(/&fmt=[^&]+/g, '').replace(/&tlang=[^&]+/g, '');
      const tlang = getYouTubeTlang(config.targetLang);
      fetch(`${cleanBase}&fmt=json3&tlang=${encodeURIComponent(tlang)}`)
        .then(r => r.text())
        .then(transRaw => {
          const transCues = parseTimedTextData(transRaw);
          if (transCues && transCues.length) {
            ytTimedTextCues.forEach(origCue => {
              const match = transCues.find(tc => Math.abs(tc.start - origCue.start) < 500);
              if (match && match.text && origCue.text) {
                origCue.translation = match.text;
                setCachedSub(origCue.text, match.text);
              }
            });
            if (ytFullTrackCues) {
              ytFullTrackCues.forEach(origCue => {
                const match = transCues.find(tc => Math.abs(tc.start - origCue.start) < 500);
                if (match && match.text && origCue.text) {
                  origCue.translation = match.text;
                  setCachedSub(origCue.text, match.text);
  ...
  function handleDetectedCaptionTracks(tracks) {
    ...
    // Fetch original and YouTube official translated track in parallel for instant 0ms full-video cache
    const origUrl = `${cleanBase}&fmt=json3`;
    const tlangUrl = `${cleanBase}&fmt=json3&tlang=${encodeURIComponent(tlang)}`;

    Promise.all([
      fetch(origUrl).then(r => r.text()).catch(() => null),
      fetch(tlangUrl).then(r => r.text()).catch(() => null)
    ]).then(([origRaw, transRaw]) => {
      const origCues = origRaw ? parseTimedTextData(origRaw) : [];
      const transCues = transRaw ? parseTimedTextData(transRaw) : [];

      if (origCues.length && transCues.length) {
        origCues.forEach(origCue => {
          const match = transCues.find(tc => Math.abs(tc.start - origCue.start) < 500);
          if (match && match.text && origCue.text) {
            origCue.translation = match.text;
            setCachedSub(origCue.text, match.text);
  ...
  function initYouTubeTimedTextPrefetch() {
    ...
    // Bi-directional Handshake: request tracks from yt-bridge.js
    const requestTracks = () => {
      try {
        window.postMessage({ source: 'holdtranslate-content', type: 'REQUEST_TRACKS' }, '*');
      } catch (e) {}
    ...
    requestTracks();
    setTimeout(requestTracks, 500);
    setTimeout(requestTracks, 1500);
  ...
  function prefetchUpcomingCues(fromMs, toMs) {
    ...
    // 1. Deep Sentence Prefetching (priority for sentence-onset 0ms instant display)
    if (ytSentenceCues && ytSentenceCues.length) {
      const targetSentences = ytSentenceCues.filter(s => s.start >= fromMs - 1000 && s.start <= toMs);
      const neededSentences = targetSentences.filter(s => !s.translation && !getCachedSub(s.text) && !pendingSubRequests.has(s.text));
      if (neededSentences.length) {
        neededSentences.slice(0, 4).forEach((sent, index) => {
          setTimeout(() => {
            if (!sent.translation && !getCachedSub(sent.text) && !pendingSubRequests.has(sent.text)) {
              translateSubtitleText(sent.text, (trans) => {
                if (trans) {
                  sent.translation = trans;
                  setCachedSub(sent.text, trans);
                }
              });
            }
          }, index * 260);
    ...
    // 2. Individual cue pool prefetch for fine-grained fallback
    ...
    // Throttle safely: max 3 requests at a time, spaced by 260ms
    needed.slice(0, 3).forEach((cue, index) => {
      setTimeout(() => {
  ...
  function stopVideoSubtitlesEngine() {
    ...
    if (ytObserverHeartbeatTimer) {
      clearInterval(ytObserverHeartbeatTimer);
      ytObserverHeartbeatTimer = null;
    }
