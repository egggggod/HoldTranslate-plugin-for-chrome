...
点击直接下载最新版 [HoldTranslate Chrome 扩展安装包](https://github.com/egggggod/HoldTranslate-plugin-for-chrome/releases/download/v1.7.4/holdtranslate-chrome-extension-v1.7.4.zip) (v1.7.4)，解压后即可在 Chrome 中体验：
...
- ⚡ **YouTube 字幕 0 延迟秒级同步与宽度解绑/行数智能对称 (Zero-Latency & Smart Line Balancing Engine)**：全面重构主世界透明桥接机制 (`yt-bridge.js`)、深度断句预翻译与底部基准线锁定（`justify-content: flex-end`）。彻底取消中文字幕受英文字幕长度限制的固有束缚，容器支持自适应居中并扩展至播放器 88% 宽度（`max-width: 88% !important; left: 50% !important; transform: translateX(-50%) !important; width: max-content !important;`）；引入智能标点与折半断行算法（`formatBalancedTranslation`），使中文行数与英文行数在观感上自然对称（英文单行时中文保持单行不折行，英文双行且中文较长时智能断为均衡双行）。
...
- 🍏 **Apple 2025 灵动玻璃岛与光影交互 (Floating Island Bar & Specular Sheen)**：底部控制栏重构为苹果 2025 最新官方软件设计风格的统一浮动晶莹药丸岛（`.floating-island-bar`），一体化整合 `[⚙️ 设置]`、`[● 就绪 · v1.7.4]` 和 `[↗ 测试页]`；引入 115° 镜面光泽掠影（Hover Specular Sheen Sweep）与 `0.97` 触觉回弹微凹陷（Tactile Press Indentation）。
...
1. **直接下载安装包（推荐 - 最新 v1.7.4）**
   - 访问我们的 **[Releases 发布页面](https://github.com/egggggod/HoldTranslate-plugin-for-chrome/releases)**（或点击直接下载：[holdtranslate-chrome-extension-v1.7.4.zip](https://github.com/egggggod/HoldTranslate-plugin-for-chrome/releases/download/v1.7.4/holdtranslate-chrome-extension-v1.7.4.zip)）；
   - 解压下载好的 `.zip` 文件；
