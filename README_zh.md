...
- **版本规范**：严格遵循 [语义化版本 (SemVer)](https://semver.org/lang/zh-CN/) 规范（`vMAJOR.MINOR.PATCH`）。
  - 当前版本：`v1.7.0`
  - `v1.7.0` 更新重点：
    - **YouTube 字幕 0 延迟秒级同步**：引入 YouTube TimedText 轨全量与向前预取机制，提前将 upcoming 35 秒字幕批次翻译存入高速 LRU 缓存池，播放到对应画面直接 0ms 瞬间命中，彻底根治时延弹出脱节；
    - **视频字幕 1:1 动态排版镜像**：动态提取原生字幕计算字号、字体族、粗细与字间距并 1:1 赋给译文，支持全屏播放与小窗缩放自适应，字体视觉浑然天成；
    - **边缘直通无穿帮弹窗体系**：最外层 `html, body` 采用成熟无感直通布局，由 Chrome 宿主窗口自身呈现原生圆角与投影，彻底根除“方背景下叠一层弧形”的角隅穿帮，内部完整保留 VisionOS 流体胶囊圆角。
  - `v1.6.2` 更新重点：
    - **移除外部多余背景方框，统一视觉审美**：移除多余的 6px 外边距与 352px 外框，回归纯净 16px 倒角；
    - **设置界面退回完整自然高度，绝不缩短**：彻底解除此前对窗口高度的 225px 硬性截断，恢复设置面板（`#settingsView`）自适应完整高度展开能力（`.settings-content` 最大 460px 舒适滚动）。
  - `v1.6.1` 更新重点：
